...
# ====== CDP 配置 ======
# 优先连接用户已有的 Chrome（已登录 Facebook），复用 session 避免被拦
CDP_URLS = [
...
def detect_cdp_url():
    ...
    for url in CDP_URLS:
        try:
            ...
            pass
    print("[CDP] 未检测到 Chrome CDP，跳过 CDP 模式")
    return None
...
def scrape_detail_playwright(library_id, wait_sec=8):
    ...
    with sync_playwright() as p:
        ...
        # 随机化浏览器指纹，让每次请求看起来像不同用户
        screen_w = random.choice([1920, 1536, 1440])
        screen_h = random.choice([1080, 900, 864])
        ...
        user_agent = (
            ...
            f"(KHTML, like Gecko) Chrome/{random.randint(115,123)}.0.0.{random.randint(0,99)} Safari/537.36"
        )
        context = browser.new_context(
            user_agent=user_agent,
            locale=random.choice(["zh-CN", "en-US", "zh-TW"]),
            viewport={"width": screen_w, "height": screen_h},
            timezone_id=random.choice(["Asia/Shanghai", "Europe/London", "America/New_York"]),
            permissions=["geolocation", "notifications"] if not HEADLESS else [],
            extra_http_headers={
        ...
        try:
            ...
            page.wait_for_load_state("load", timeout=15000)
            # 等广告内容加载完成（等待"查看广告详情"按钮出现，最多等15秒）
            try:
                page.wait_for_selector('button:has-text("查看广告详情"), [role="button"]:has-text("查看广告详情"), a:has-text("查看广告详情")', timeout=15000)
                print("  [Playwright] 已切换 Facebook 语言到简体中文")
            except Exception:
                print("  [Playwright] 按钮加载超时，继续")
            page.wait_for_timeout(3000)  # 额外等3秒确保内容完全渲染
        except Exception as e:
            print(f"  [Playwright] 语言切换失败: {e}")

        # ---- 步骤2: 点击"查看广告详情" ----
        # 尝试两种策略：get_by_text 和 locator.filter
        btn_keywords = [
            '\u67e5\u770b\u5e7f\u544a\u8be6\u60c5',              # 查看广告详情
            '\u67e5\u770b\u5e7f\u544a\u8be6\u60c5\uff08\u539f\u59cb\u7248\uff09',  # 查看广告详情（原始版）
            'View ad details', 'View Ad Details', 'Ad Details',
        ]
        clicked = False

        # 策略1：get_by_text（整页搜索）
        for kw in btn_keywords:
            try:
                page.get_by_text(kw, exact=False).first.click(timeout=5000, force=True)
                print(f"  [Playwright] 点击按钮(策略1): {kw}")
                clicked = True
                break
            except Exception:
                pass

        # 策略2：逐个 dialog 内部用 locator 找 button
        if not clicked:
            for dialog in page.locator('[role="dialog"]').all():
                for kw in btn_keywords:
                    try:
                        ...
                        print(f"  [Playwright] 点击按钮(策略2 dialog): {kw}")
                        clicked = True
                        break
                    except Exception:
                        pass
        ...
        # 策略3：直接在页面找 button 元素
        if not clicked:
            for kw in btn_keywords:
                try:
                    page.locator(f'button:has-text("{kw}"), [role="button"]:has-text("{kw}")').first.click(timeout=5000, force=True)
                    print(f"  [Playwright] 点击按钮(策略3): {kw}")
                    clicked = True
                    break
                except Exception:
                    pass
        ...
        # ---- 步骤3: 等待弹窗出现 ----
        # 点击后弹出一个 popup，里面有"查看广告详情"按钮，需要再点一次
        try:
        ...
        # 在弹窗内找"查看广告详情"按钮并点击（第二层），用同样的三策略
        btn_in_dialog = False
        for dialog in page.locator('[role="dialog"]').all():
            for kw in btn_keywords:
                try:
                    dialog.locator(f'button, [role="button"]').filter(has_text=kw).first.click(timeout=5000, force=True)
                    print(f"  [Playwright] 弹窗内点击(dialog策略): {kw}")
                    btn_in_dialog = True
                    break
                except Exception:
                    pass
        ...
        if not btn_in_dialog:
            for kw in btn_keywords:
                try:
                    page.get_by_text(kw, exact=False).first.click(timeout=5000, force=True)
                    print(f"  [Playwright] 弹窗内点击(全局策略): {kw}")
                    btn_in_dialog = True
                    break
                except Exception:
                    pass
        ...
        tab_labels = [
            ...
            '\u95dc\u65bc\u5e7f\u544a\u4e3b',                                          # 關於廣告主
            '\u5e7f\u544a\u4e3b\u548c\u4ed8\u8d39\u65b9',                             # 廣吿主和付費方
            'Ad details by location', 'About the advertiser', 'Advertiser Info',
        ]
...
def main():
    ...
    for i, ad in enumerate(to_scrape, 1):
        ...
        # 每抓完一个随机等3-8秒再处理下一个（防止频率过高被Facebook拦截）
